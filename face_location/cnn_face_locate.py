...
from emotion_distinguish_locate.util import read_images_list, unify_image
...
class Dataset:
    ...
    def load(self, img_rows=IMAGE_SIZE, img_cols=IMAGE_SIZE,
             img_channels=3):
        # 加载数据集到内存
        images, labels, face_num = read_images_list(self.path_name)
...
class Model:
    ...
    def face_predict(self, image):
        ...
        if K.image_dim_ordering() == 'th' and image.shape != (1, 3, IMAGE_SIZE, IMAGE_SIZE):
            image = unify_image(image)  # 尺寸必须与训练集一致都应该是IMAGE_SIZE x IMAGE_SIZE
